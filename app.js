...
app.post("/insetar", async (req, res) => {
  ...
  try {
    ...
    const [verificar] = await connect.execute(
      "SELECT * FROM usuario WHERE NumeroDocumento=? AND TipoDocumento=?",
    ...
    if (verificar.length > 0) {
      ...
      await connect.execute(
        "INSERT INTO usuario (Nombre, Apellido, Correo, Telefono,TipoDocumento,NumeroDocumento,MANZANAId_manzana) VALUES (?,?,?,?,?,?,?) ",
        [
          Nombre,
          Apellido,
          Correo,
...
app.post("/Iniciar", async (req, res) => {
  ...
  try {
    const connect = await mysql2.createConnection(db);
    const [verificarInicio] = await connect.execute(
      "SELECT * FROM usuario WHERE NumeroDocumento=? AND TipoDocumento=?",
      [NumeroDocumento, TipoDocumento]
    );
    if (verificarInicio.length > 0) {
      const [man] = await connect.execute(
        "SELECT manzana.Nombre FROM usuario INNER JOIN manzana ON usuario.Id_usuario = manzana.Id_manzana WHERE usuario.Nombre = ?",
        [verificarInicio[0].Nombre]
      );
...
app.post("/IniciarAdmin", async (req, res) => {
  ...
  try {
    const connect = await mysql2.createConnection(db);
    const [verificaradmin] = await connect.execute(
      "SELECT * FROM usuario WHERE NumeroDocumento=? AND TipoDocumento=?",
      [NumeroDocumento, TipoDocumento]
    );
...
app.post("/obtener-servicios-usuario", async (req, res) => {
  ...
  try {
    const connect = await mysql2.createConnection(db);
    const [datos] = await connect.execute(
      'SELECT servicio.Nombre FROM servicio INNER JOIN manzana_servicio ON manzana_servicio.ServicioId_servicio = servicio.Id_servicio INNER JOIN manzana ON manzana.Id_manzana = manzana_servicio.MANZANAId_manzana INNER JOIN usuario ON manzana.Id_manzana=usuario.MANZANAId_manzana WHERE usuario.Nombre="juan david";'
    );
    console.log(datos);
...
app.post("/obtener-servicios-admin", async (req, res) => {
  ...
  try {
    const connect = await mysql2.createConnection(db);
    const [datos] = await connect.execute(
      "SELECT DISTINCT Nombre FROM servicio;"
    );
    console.log(datos);
    res.json({ servicios: datos.map((hijo) => hijo.Nombre) });
    await connect.end();
...
app.post("/guardar-servicio-usuario", async (req, res) => {
  const usuario = req.session.USUARIOId_usuario;
  ...
  try {
    ...
    const [IDU] = await conect.execute(
      "SELECT Id_usuario FROM usuario WHERE NumeroDocumento=?",
      [NumeroDocumento]
    );
    await conect.execute(
      "INSERT INTO solicitud (Nombre,FechaHora,USUARIOId_usuario) values(?,?,?)",
      [servicio, FechaHora, IDU[0].usuario]
    );
...
app.post("/Agregar-Manzana", async (req, res) => {
  ...
  try {
    ...
    const connect = await mysql2.createConnection(db);
    const [verificar] = await connect.execute(
      "SELECT * FROM manzana WHERE Nombre=? AND Direccion=? AND Localidad=?",
    ...
    if (verificar.length > 0) {
      ...
      await connect.execute(
        "INSERT INTO manzana (Nombre,Direccion,Localidad) VALUES (?,?,?) ",
        [Nombre, Direccion, Localidad]
      );
      res.status(201).send(`
                <script>
...
app.post("/Eliminar-Manzana", async (req, res) => {
  ...
  try {
    ...
    const [verificar] = await connect.execute(
      "SELECT * FROM manzana WHERE Nombre=?",
      [Nombre]
    );
...
app.post("/Agregar-Servicio", async (req, res) => {
  ...
  try {
    ...
    const connect = await mysql2.createConnection(db);
    const [verificar] = await connect.execute(
      "SELECT * FROM servicio WHERE Nombre=? AND Descripcion=?",
      [Nombre, Descripcion]
    );
    if (verificar.length > 0) {
      res.status(409).send(`
...
app.post("/Eliminar-Servicio", async (req, res) => {
  ...
  try {
    ...
    // Verificar si la manzana existe (si es necesario)
    const [verificar] = await connect.execute(
      "SELECT * FROM servicio WHERE Nombre=?",
      [Nombre]
    );
...
app.post("/obtener-usuarios-admin", async (req, res) => {
  ...
  try {
    const connect = await mysql2.createConnection(db);
    const [datos] = await connect.execute(
      "SELECT DISTINCT Nombre, Apellido, NumeroDocumento, Id_usuario FROM usuario;"
    );
    console.log(datos);
    res.json({
      usuario: datos.map((hijo) => ({
        Id_usuario: hijo.Id_usuario,
        Nombre: hijo.Nombre,
...
app.delete("/eliminar-solicitud", async (req, res) => {
  ...
  console.log("EJECUTANDO ELIMINAR SOLICITUD:", solicitud);
  try {
    const connect = await mysql2.createConnection(db);

    connect
      .execute("DELETE FROM solicitud WHERE Id_solicitud=?", [solicitud])
      .then((res) => {
        console.log("res 1: ", res);
        /* connect
          .execute("DELETE FROM usuario WHERE Id_usuario=?", [solicitud])
          .then((res) => {
...
//eliminar usuario
app.delete("/eliminar-usuarios", async (req, res) => {
  const usuario = req.body.Id_usuario;
  console.log("EJECUTANDO ELIMINAR USUARIO:", usuario);
  try {
    const connect = await mysql2.createConnection(db);

    connect
      .execute("DELETE FROM solicitud WHERE USUARIOId_usuario=?", [usuario])
      .then((res) => {
        console.log("res 1: ", res);
        connect
          .execute("DELETE FROM usuario WHERE Id_usuario=?", [usuario])
          .then((res) => {
            console.log("res 2: ", res);
          })
          .then((res) => {
